    ...
    for ite in tqdm(range(int(maxiter))):
        if ite % update_interval == 0:
            ...
            if ite > 0 and delta_label < tol or ite > cfg.maxiter:
                ...
                model.save_weights(
                    os.path.join(path_models_dcec, 'dec_model_final.h5'))
        ...
        if ite % save_interval == 0:
            ...
            model.save_weights(
                os.path.join(
                    exp, path_models_dcec, 'dec_model_'+str(ite)+'.h5'))
        ite += 1

    # Save metrics to csv
    df = pd.DataFrame(data=dictionary)
    try:
        os.remove(os.path.join(tables, exp, 'dec_train_metrics.csv'))
    except:
        pass
    df.to_csv(os.path.join(
        tables, exp, 'dec_train_metrics.csv'), index=False)
