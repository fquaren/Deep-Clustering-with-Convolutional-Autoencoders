...
def CAE_Conv2DTranspose_small(input_shape=(192, 192, 1), filters=[16, 32, 300]):
    ...
    # Encoder
    x = Conv2D(filters[0], 7, strides=4, padding='same', activation='relu',name='conv1', input_shape=input_shape)(input_img)
    x = Conv2D(filters[1], 3, strides=2, padding='same',activation='relu', name='conv2')(x)
    x = Flatten(name='flatten_1')(x)
    ...
    decoded = Conv2DTranspose(1, 7, strides=4, padding='same', name='deconv1')(x)
